...
public class MEConfig {
    ...
    private void saveDisabled() {
        ...
        try {
            disFile.set("disabled-for", Emotes.DeclinedPlayers);
            disFile.save(file);
        } catch (IOException e) {
    ...
    private void loadConfigMojis() {
        ...
        for (List<String> aliases : Emotes.emojis.keySet()) {
            ...
            if (!newAliases.isEmpty()) {
                String Symbol = Emotes.emojis.get(aliases);
        ...
        if (!NewEmojis.isEmpty()) {
            Emotes.emojis.clear();
            Emotes.emojis.putAll(NewEmojis);
        }
        ...
        if (!file.exists()) {
            ...
            try {
                ...
                if (disFile.getStringList("disabled-for").size() > 0) {
                    Emotes.DeclinedPlayers.addAll(disFile.getStringList("disabled-for"));
                }
            } catch (Exception e) {
                e.printStackTrace();
            }
